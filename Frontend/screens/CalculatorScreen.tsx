...
const CalculatorScreen: React.FC = () => {
  const [constructionType, setConstructionType] = useState('Balcony');
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const calculatedPrice = includeTax
        ? response.data.priceWithTax
        : response.data.priceWithoutTax;

      // Przygotowanie wymiarów w zależności od parametrów
      const dimensions = [];
      if (fields.Height) dimensions.push(`Height: ${fields.Height}m`);
      if (fields.Length) dimensions.push(`Length: ${fields.Length}m`);
      if (fields.Width) dimensions.push(`Width: ${fields.Width}m`);
      if (fields.Thickness) dimensions.push(`Thickness: ${fields.Thickness}m`);
      if (fields.Area) dimensions.push(`Area: ${fields.Area}m`);
      if (fields.Count) dimensions.push(`Count: ${fields.Count}`);
      if (fields.Amount) dimensions.push(`Amount: ${fields.Amount}`);
      if (fields.SurfaceArea) dimensions.push(`Surface Area: ${fields.SurfaceArea}`);
      if (fields.Depth) dimensions.push(`Depth: ${fields.Depth}`);
      if (fields.WallSurfaceArea) dimensions.push(`Wall Surface Area: ${fields.WallSurfaceArea}`);
      if (fields.NumberOfSteps) dimensions.push(`Number Of Steps: ${fields.NumberOfSteps}`);
      const dimensionsText = dimensions.length > 0 ? dimensions.join(', ') : 'N/A';
      ...
      navigation.navigate('CostSummary', {
        type: constructionType,
        parameters: fields, // Przekazanie parametrów
        calculatedPrice,
        includeTax,
        dimensions: dimensionsText, // Wymiary
        details, // Szczegóły
      } as any); // Użycie 'any' na końcu      
    } catch (error: any) {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.switchContainer}>
        <Text style={styles.switchLabel}>
          {includeTax ? 'Show Price With Tax' : 'Show Price Without Tax'}
        </Text>
        <Switch
          value={includeTax}
          onValueChange={(value) => setIncludeTax(value)}
        />
      </View>
